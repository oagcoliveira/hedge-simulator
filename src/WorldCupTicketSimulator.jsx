...
const WorldCupTicketSimulator = () => {
  ...
  const stageOdds = useMemo(() => {
    const data = {};
    const vigFactor = oddsData.totalImplied > 0 ? 100 / oddsData.totalImplied : 1;
    let cumProb = 0;
    for (const stage of STAGES) {
      ...
      const adjustedOdds = fairOdds * vigFactor;
  ...
  const scenarios = useMemo(() => {
    ...
    for (let i = 0; i < STAGES.length; i++) {
      ...
      const cashFlows = [
        ...
        { month: proceedsMonth, amount: grossProceeds + hedgeReturn },
      ];
    ...
    const allStakes = STAGES.reduce((sum, s) => sum + hedgeStakes[s], 0);
    const hCarryCostFinals = hedgeCarryCostPerStage(STAGES);
    const hedgeResultFinals = -allStakes;
  ...
  const breakEvenResale = useMemo(() => {
    ...
    const targetNet = totalPurchase + carryingCost + hcc + allStakes;
    return (targetNet + fixedTransactionCost) / (numTickets * netFeeRate);
  }, [totalPurchase, carryingCost, numTickets, resaleFeePercent, processingFeePercent,
      fixedTransactionCost, hedgeStakes, annualOpportunityCost]);
  ...
  const sensitivityData = useMemo(() => {
    ...
    for (let price = 2000; price <= 30000; price += 500) {
      ...
      const finalsPL = np - totalPurchase - carryingCost - hcc - allStakes;
    ...
    return data;
      ...
      totalPurchase, carryingCost, hedgeStakes, annualOpportunityCost]);

  const maxLoss = Math.min(...scenarios.map(s => s.netPL));
  const maxGain = Math.max(...scenarios.map(s => s.netPL));

  // Perfect hedge: solve for G such that all unlocked scenarios have equal P&L
  // Recursion: s_i = (G + CC + S_{i-1}) / (o_i - 1), where S_{i-1} = sum of prior stakes
  // Locked stages contribute a fixed stake to S_{i-1} instead of being solved for
  // S_5 is still linear in G → solve Finals equation analytically
  const perfectHedgeData = useMemo(() => {
    const runRecursion = (G) => {
      ...
      for (const stage of STAGES) {
        if (lockedStages[stage]) {
          stakes[stage] = hedgeStakes[stage];
          cumStakes += hedgeStakes[stage];
          continue;
        }
    ...
    const roundedStakes = {};
    for (const stage of STAGES) {
      roundedStakes[stage] = lockedStages[stage] ? hedgeStakes[stage] : Math.max(0, Math.round(final.stakes[stage]));
    }
    return { G, stakes: roundedStakes };
  }, [stageOdds, carryingCost, expectedResale, numTickets, resaleFeePercent, processingFeePercent,
      fixedTransactionCost, totalPurchase, lockedStages, hedgeStakes]);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-xl shadow-2xl p-8 mb-8">
          ...
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-6 mb-8 border-2 border-amber-200">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Hedge Module (Stage-by-Stage Bets Against Brazil)</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                ...
                <div className="bg-white rounded p-4">
                  ...
                  <div className="text-lg font-bold text-indigo-600">{probFinals.toFixed(1)}%</div>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-amber-300">
                      ...
                      <th className="text-center py-2 px-2 font-semibold text-gray-700">Lock</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={key} className={`border-b border-amber-100 ${isFinalsGroup ? 'bg-green-50' : ''} ${isLocked ? 'bg-amber-100/50' : ''}`}>
                          ...
                          <td className="py-2 px-2 text-right">
                            {isHedgeable ? (
                              <input
                                type="number"
                                step={key === 'Group' || key === 'R32' ? '10' : '100'}
                                value={hedgeStakes[key]}
                                onChange={(e) => updateHedgeStake(key, e.target.value)}
                                disabled={isLocked}
                                className="w-24 px-2 py-1 border border-gray-300 rounded text-right focus:ring-2 focus:ring-amber-500 focus:border-transparent disabled:bg-gray-100 disabled:text-gray-500"
                              />
                            ) : (
                  ...
                  {scenarios.map(s => (
                    <tr key={s.stage} className={`border-b ${s.isFinals ? 'bg-green-50 font-semibold' : ''}`}>
                      ...
                      <td className={`py-3 px-3 text-right font-semibold ${s.hedgeResult >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {formatCurrency(s.hedgeResult)}
                      </td>
                      <td className="py-3 px-3 text-right text-red-600">{s.hCarryCost > 0 ? `-${formatCurrency(s.hCarryCost)}` : '-'}</td>
          ...
          <div className="border border-gray-200 rounded-lg mb-8">
            <button
              ...
              className="w-full flex items-center justify-between px-6 py-4 text-left bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
            >
              <span className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Advanced Inputs</span>
              <span className="text-gray-400 text-lg">{inputsExpanded ? '▲' : '▼'}</span>
            </button>
            {inputsExpanded && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
                {/* Ticket Purchase */}
                <div>
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Ticket Purchase</h3>
                  <div className="space-y-4">
                    <div>
                ...
                {/* Transaction Costs */}
                <div>
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Transaction Costs</h3>
                  <div className="space-y-4">
                    <div>
                ...
                {/* Carrying Cost */}
                <div>
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Carrying / Opportunity Cost</h3>
                  <div className="space-y-4">
                    <div>
